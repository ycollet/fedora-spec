...
%global commit0 be55cf1058ae9bcb28456ef8e1615cc49227feed
...
Source0: Rack.tar.gz
Source1: https://github.com/gluethegiant/gtg-rack/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: GlueTheGiant_plugin.json
...
%description
GlueTheGiant plugin for Rack.
Stereo mixer to three stereo buses
...
mkdir GlueTheGiant_plugin
tar xvfz %{SOURCE1} --directory=GlueTheGiant_plugin --strip-components=1 

cp -n %{SOURCE2} GlueTheGiant_plugin/plugin.json

%build

cd GlueTheGiant_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/GlueTheGiant/
cp -r GlueTheGiant_plugin/dist/GlueTheGiant/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/GlueTheGiant/
