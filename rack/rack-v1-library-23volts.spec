...
Source0: Rack.tar.gz
Source1: https://github.com/23volts/23volts-vcv/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: 23volts_plugin.json
...
mkdir 23volts_plugin
tar xvfz %{SOURCE1} --directory=23volts_plugin --strip-components=1 

cp -n %{SOURCE2} 23volts_plugin/plugin.json
...
mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/23volts/
cp -r 23volts_plugin/dist/23volts/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/23volts/
