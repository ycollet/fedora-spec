...
Source0: Rack.tar.gz
Source1: https://github.com/ODDSound/MTS-ESP-VCVRack/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: ODDSound_MTS_ESP_plugin.json
...
%description
ODDSound_MTS_ESP plugin for Rack.
...
mkdir ODDSound_MTS_ESP_plugin
tar xvfz %{SOURCE1} --directory=ODDSound_MTS_ESP_plugin --strip-components=1 

cp -n %{SOURCE2} ODDSound_MTS_ESP_plugin/plugin.json

%build

cd ODDSound_MTS_ESP_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ODDSound_MTS_ESP/
cp -r ODDSound_MTS_ESP_plugin/dist/ODDSound_MTS_ESP/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ODDSound_MTS_ESP/
