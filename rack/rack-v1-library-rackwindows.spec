...
%global commit0 1b639e389aab543a0b1a4eaaee68b3be0f68b5d4
...
Source0: Rack.tar.gz
Source1: https://github.com/n0jo/rackwindows/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: rackwindows_plugin.json
...
mkdir rackwindows_plugin
tar xvfz %{SOURCE1} --directory=rackwindows_plugin --strip-components=1 

cp -n %{SOURCE2} rackwindows_plugin/plugin.json

%build

cd rackwindows_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/rackwindows/
cp -r rackwindows_plugin/dist/rackwindows/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/rackwindows/
