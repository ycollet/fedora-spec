...
%global commit0 90b1ef13f4419808bf35b8317d86c68deb01052f
...
Source0: Rack.tar.gz
Source1: https://github.com/mhetrick/nonlinearcircuits/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: NonlinearCircuits_plugin.json
...
%description
NonlinearCircuits plugin for Rack.
Dual sequencer and clock divider. Panel by Jürgen Krehan.
...
mkdir NonlinearCircuits_plugin
tar xvfz %{SOURCE1} --directory=NonlinearCircuits_plugin --strip-components=1 

cp -n %{SOURCE2} NonlinearCircuits_plugin/plugin.json

%build

cd NonlinearCircuits_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/NonlinearCircuits/
cp -r NonlinearCircuits_plugin/dist/NonlinearCircuits/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/NonlinearCircuits/
