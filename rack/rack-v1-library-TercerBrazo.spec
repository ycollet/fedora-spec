...
%global commit0 d6797cd4c63eded9b3b99c495c9adfa93372071d
...
Source0: Rack.tar.gz
Source1: https://github.com/diegodorado/TercerBrazo/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: TercerBrazo_plugin.json
...
%description
TercerBrazo plugin for Rack.
Sega Genesis chip based synth
...
mkdir TercerBrazo_plugin
tar xvfz %{SOURCE1} --directory=TercerBrazo_plugin --strip-components=1 

cp -n %{SOURCE2} TercerBrazo_plugin/plugin.json

%build

cd TercerBrazo_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/TercerBrazo/
cp -r TercerBrazo_plugin/dist/TercerBrazo/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/TercerBrazo/
