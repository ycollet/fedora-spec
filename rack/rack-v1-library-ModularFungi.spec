...
%global commit0 f64ab899a7fe3b192fdcb8e359244187be7eb880
...
Source0: Rack.tar.gz
Source1: https://github.com/david-c14/ModularFungi/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: ModularFungi_plugin.json
...
mkdir ModularFungi_plugin
tar xvfz %{SOURCE1} --directory=ModularFungi_plugin --strip-components=1 

cp -n %{SOURCE2} ModularFungi_plugin/plugin.json

%build

cd ModularFungi_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ModularFungi/
cp -r ModularFungi_plugin/dist/ModularFungi/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ModularFungi/
