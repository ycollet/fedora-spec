...
%global commit0 e8986474a9bfc01e4afcc3bc2a1490679a518b67
...
Name:    rack-v1-rchoices
Version: 1.0.0
Release: 3%{?dist}
Summary: rchoices plugin for Rack
License: GPLv2+
URL:     https://github.com/davekch/rchoices

# ./rack-source.sh <tag>
# ./rack-source.sh v1.1.6

Source0: Rack.tar.gz
Source1: https://github.com/davekch/rchoices/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: rchoices_plugin.json
...
mkdir rchoices_plugin
tar xvfz %{SOURCE1} --directory=rchoices_plugin --strip-components=1 

cp -n %{SOURCE2} rchoices_plugin/plugin.json
...
mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/rchoices/
cp -r rchoices_plugin/dist/rchoices/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/rchoices/
