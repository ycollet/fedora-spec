...
%global commit0 6c0bd7fbe8bcb6f563c7bd8d66d59c2b739791ac
...
Source0: Rack.tar.gz
Source1: https://github.com/LOGUNIVPM/LOGinstruments/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: LOGinstruments_plugin.json
...
%description
LOGinstruments plugin for Rack.
...
mkdir LOGinstruments_plugin
tar xvfz %{SOURCE1} --directory=LOGinstruments_plugin --strip-components=1 

cp -n %{SOURCE2} LOGinstruments_plugin/plugin.json

%build

cd LOGinstruments_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/LOGinstruments/
cp -r LOGinstruments_plugin/dist/LOGinstruments/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/LOGinstruments/
