...
Source2: AaronStatic_plugin.json
...
mkdir AaronStatic_plugin
tar xvfz %{SOURCE1} --directory=AaronStatic_plugin --strip-components=1 

cp -n %{SOURCE2} AaronStatic_plugin/plugin.json

%build

cd AaronStatic_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AaronStatic/
cp -r AaronStatic_plugin/dist/AaronStatic/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AaronStatic/
