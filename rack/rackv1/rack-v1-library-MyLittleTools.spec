...
%global commit0 7fc84bbb62962f7b03c175f6192a9d1c2b22fb7e
...
Source0: Rack.tar.gz
Source1: https://github.com/digitalhappens/MyLittleTools/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: MyLittleTools_plugin.json
...
mkdir MyLittleTools_plugin
tar xvfz %{SOURCE1} --directory=MyLittleTools_plugin --strip-components=1 

cp -n %{SOURCE2} MyLittleTools_plugin/plugin.json

%build

cd MyLittleTools_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/MyLittleTools/
cp -r MyLittleTools_plugin/dist/MyLittleTools/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/MyLittleTools/
