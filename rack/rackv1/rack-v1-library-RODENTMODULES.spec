...
%global commit0 465f7c9c774b779583b7411de10b7aa54083e4da
...
Source0: Rack.tar.gz
Source1: https://github.com/RODENTCAT/RODENTMODULES/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: RODENTMODULES_plugin.json
...
mkdir RODENTMODULES_plugin
tar xvfz %{SOURCE1} --directory=RODENTMODULES_plugin --strip-components=1 

cp -n %{SOURCE2} RODENTMODULES_plugin/plugin.json

%build

cd RODENTMODULES_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/RODENTMODULES/
cp -r RODENTMODULES_plugin/dist/RODENTMODULES/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/RODENTMODULES/
