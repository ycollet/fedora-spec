...
Source0: Rack.tar.gz
Source1: https://github.com/countmodula/VCVRackLunettaModula/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: LunettaModula_plugin.json
...
mkdir LunettaModula_plugin
tar xvfz %{SOURCE1} --directory=LunettaModula_plugin --strip-components=1 

cp -n %{SOURCE2} LunettaModula_plugin/plugin.json

%build

cd LunettaModula_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/LunettaModula/
cp -r LunettaModula_plugin/dist/LunettaModula/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/LunettaModula/
