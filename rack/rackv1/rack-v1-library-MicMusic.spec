...
Source0: Rack.tar.gz
Source1: https://github.com/very-cool-name/MicMusic-VCV/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: MicMusic_plugin.json
...
mkdir MicMusic_plugin
tar xvfz %{SOURCE1} --directory=MicMusic_plugin --strip-components=1 

cp -n %{SOURCE2} MicMusic_plugin/plugin.json
...
mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/MicMusic/
cp -r MicMusic_plugin/dist/MicMusic/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/MicMusic/
