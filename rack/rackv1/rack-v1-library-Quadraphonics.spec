...
%global commit0 96885ce5351d06ba3e603e4d29c4403f23edc53e
...
Source0: Rack.tar.gz
Source1: https://github.com/KoreTeknology/Quadraphonic-Plugins-for-VCV-Rack/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: Quadraphonics_plugin.json
...
mkdir Quadraphonics_plugin
tar xvfz %{SOURCE1} --directory=Quadraphonics_plugin --strip-components=1 

cp -n %{SOURCE2} Quadraphonics_plugin/plugin.json

%build

cd Quadraphonics_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/Quadraphonics/
cp -r Quadraphonics_plugin/dist/Quadraphonics/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/Quadraphonics/
