...
%global commit0 98a9448e9872f51b984c690387dbcfc1c15b8553
...
Source0: Rack.tar.gz
Source1: https://github.com/pknodle/WeldSplatter/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: WeldSplatter_plugin.json
...
mkdir WeldSplatter_plugin
tar xvfz %{SOURCE1} --directory=WeldSplatter_plugin --strip-components=1 

cp -n %{SOURCE2} WeldSplatter_plugin/plugin.json

%build

cd WeldSplatter_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/WeldSplatter/
cp -r WeldSplatter_plugin/dist/WeldSplatter/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/WeldSplatter/
