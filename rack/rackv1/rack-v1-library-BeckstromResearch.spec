...
%global commit0 017fb891331f39d7a182ad1a6febe28567a1a00d
...
Name:    rack-v1-BeckstromResearch
Version: 0.6.0
Release: 3%{?dist}
Summary: BeckstromResearch plugin for Rack
License: GPLv2+
...
Source0: Rack.tar.gz
Source1: https://github.com/chrisbeckstrom/Beckstrom_Research_Rack/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: BeckstromResearch_plugin.json
...
%description
BeckstromResearch plugin for Rack.
...
mkdir BeckstromResearch_plugin
tar xvfz %{SOURCE1} --directory=BeckstromResearch_plugin --strip-components=1 

cp -n %{SOURCE2} BeckstromResearch_plugin/plugin.json

%build

cd BeckstromResearch_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/BeckstromResearch/
cp -r BeckstromResearch_plugin/dist/BeckstromResearch/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/BeckstromResearch/
