...
Name:    rack-v1-WhatTheRack
Version: 1.1.1
Release: 3%{?dist}
Summary: WhatTheRack plugin for Rack

License: GPLv2+
URL:     https://github.com/korfuri/WhatTheRack
...
Source0: Rack.tar.gz
Source1: https://github.com/korfuri/WhatTheRack/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: WhatTheRack_plugin.json
...
mkdir WhatTheRack_plugin
tar xvfz %{SOURCE1} --directory=WhatTheRack_plugin --strip-components=1 

cp -n %{SOURCE2} WhatTheRack_plugin/plugin.json

%build

cd WhatTheRack_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/WhatTheRack/
cp -r WhatTheRack_plugin/dist/WhatTheRack/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/WhatTheRack/
