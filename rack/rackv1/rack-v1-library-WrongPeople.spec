...
%global commit0 74c8d2bd7246813ff60887e0bd9068a84b7b21b3
...
Name:    rack-v1-WrongPeople
Version: 1.0.1
Release: 3%{?dist}
Summary: WrongPeople plugin for Rack
License: GPLv2+
URL:     https://github.com/WrongPeople/WrongPeopleVCV
...
mkdir WrongPeople_plugin
tar xvfz %{SOURCE1} --directory=WrongPeople_plugin --strip-components=1 

cp -n %{SOURCE2} WrongPeople_plugin/plugin.json

%build

cd WrongPeople_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/WrongPeople/
cp -r WrongPeople_plugin/dist/WrongPeople/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/WrongPeople/
