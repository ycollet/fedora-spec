...
%global commit0 12cac86eaed62fcf27b4c6c32d4c6f5e951f298c

%global gittag0 0.6.2
%global shortcommit0 %(c=%{commit0}; echo ${c:0:7})

# Disable production of debug package.
%global debug_package %{nil}

Name:    rack-v1-SubmarineUtility
Version: 0.6.2
Release: 3%{?dist}
...
mkdir SubmarineUtility_plugin
tar xvfz %{SOURCE1} --directory=SubmarineUtility_plugin --strip-components=1 

cp -n %{SOURCE2} SubmarineUtility_plugin/plugin.json

%build

cd SubmarineUtility_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/SubmarineUtility/
cp -r SubmarineUtility_plugin/dist/SubmarineUtility/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/SubmarineUtility/

%files
%{_libexecdir}/*

%changelog
* Tue Feb 11 2020 Yann Collette <ycollette.nospam@free.fr> - 0.6.2-3
- initial specfile
