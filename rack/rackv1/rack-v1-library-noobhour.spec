...
Source0: Rack.tar.gz
Source1: https://github.com/NicolasNeubauer/noobhour_modules/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: noobhour_plugin.json
...
mkdir noobhour_plugin
tar xvfz %{SOURCE1} --directory=noobhour_plugin --strip-components=1 

cp -n %{SOURCE2} noobhour_plugin/plugin.json
...
mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/noobhour/
cp -r noobhour_plugin/dist/noobhour/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/noobhour/
