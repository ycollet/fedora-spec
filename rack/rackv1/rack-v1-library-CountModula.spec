...
%global commit0 5c99b60cd92d72e3ec4f2789195821af50d9fce4

%global gittag0 1.13.0
...
Source0: Rack.tar.gz
Source1: https://github.com/countmodula/VCVRackPlugins/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: CountModula_plugin.json
...
mkdir CountModula_plugin
tar xvfz %{SOURCE1} --directory=CountModula_plugin --strip-components=1 

cp -n %{SOURCE2} CountModula_plugin/plugin.json

%build

cd CountModula_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/CountModula/
cp -r CountModula_plugin/dist/CountModula/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/CountModula/

%files
%{_libexecdir}/*

%changelog
* Tue Feb 11 2020 Yann Collette <ycollette.nospam@free.fr> - 1.13.0-3
- initial specfile
