...
%global commit0 9c107d3ea51cbffc586fdff806629bfb22a0e2b6

%global gittag0 1.1.8
...
mkdir ImpromptuModular_plugin
tar xvfz %{SOURCE1} --directory=ImpromptuModular_plugin --strip-components=1 

cp -n %{SOURCE2} ImpromptuModular_plugin/plugin.json

%build

cd ImpromptuModular_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ImpromptuModular/
cp -r ImpromptuModular_plugin/dist/ImpromptuModular/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ImpromptuModular/

%files
%{_libexecdir}/*

%changelog
* Tue Feb 11 2020 Yann Collette <ycollette.nospam@free.fr> - 1.1.8-3
- initial specfile
