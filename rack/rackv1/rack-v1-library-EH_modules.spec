...
%global commit0 fc16afd2259a0873e9d53cba47717ace749e8fe8
...
Source0: Rack.tar.gz
Source1: https://github.com/eh2k/fv1-emu/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: EH_modules_plugin.json
...
mkdir EH_modules_plugin
tar xvfz %{SOURCE1} --directory=EH_modules_plugin --strip-components=1 

cp -n %{SOURCE2} EH_modules_plugin/plugin.json

%build

cd EH_modules_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/EH_modules/
cp -r EH_modules_plugin/dist/EH_modules/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/EH_modules/
