...
%global commit0 62e4782be2b9eeb4886b4b71092dad78de9c7446
...
Source0: Rack.tar.gz
Source1: https://github.com/expertsleepersltd/vcvrack-encoders/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: ExpertSleepers-Encoders_plugin.json
...
cd ExpertSleepers-Encoders_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ExpertSleepers-Encoders/
cp -r ExpertSleepers-Encoders_plugin/dist/ExpertSleepers-Encoders/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ExpertSleepers-Encoders/
