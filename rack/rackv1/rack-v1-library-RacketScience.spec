...
%global commit0 04caab44e47d60b3af2a9fae0108e9cc241f31ac
...
Source0: Rack.tar.gz
Source1: https://github.com/ContemporaryInsanity/RacketScience/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: RacketScience_plugin.json
...
mkdir RacketScience_plugin
tar xvfz %{SOURCE1} --directory=RacketScience_plugin --strip-components=1 

cp -n %{SOURCE2} RacketScience_plugin/plugin.json

%build

cd RacketScience_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/RacketScience/
cp -r RacketScience_plugin/dist/RacketScience/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/RacketScience/
