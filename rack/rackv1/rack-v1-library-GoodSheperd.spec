...
Source0: Rack.tar.gz
Source1: https://github.com/jensschulze/GoodSheperd/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: GoodSheperd_plugin.json
...
mkdir GoodSheperd_plugin
tar xvfz %{SOURCE1} --directory=GoodSheperd_plugin --strip-components=1 

cp -n %{SOURCE2} GoodSheperd_plugin/plugin.json

%build

cd GoodSheperd_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/GoodSheperd/
cp -r GoodSheperd_plugin/dist/GoodSheperd/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/GoodSheperd/
