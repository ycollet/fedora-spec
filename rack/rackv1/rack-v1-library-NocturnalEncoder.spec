...
License: GPLv2+
URL:     https://github.com/djpeterso23662/NocturnalEncoder
...
Source0: Rack.tar.gz
Source1: https://github.com/djpeterso23662/NocturnalEncoder/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: NocturnalEncoder_plugin.json
...
%description
NocturnalEncoder plugin for Rack.
...
cd NocturnalEncoder_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/NocturnalEncoder/
cp -r NocturnalEncoder_plugin/dist/NocturnalEncoder/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/NocturnalEncoder/
