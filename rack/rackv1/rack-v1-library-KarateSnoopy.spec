...
%global commit0 4168a22c4fd90790697f8a2416472fe257153294
...
Source0: Rack.tar.gz
Source1: https://github.com/KarateSnoopy/vcv-karatesnoopy.git/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: KarateSnoopy_plugin.json
...
mkdir KarateSnoopy_plugin
tar xvfz %{SOURCE1} --directory=KarateSnoopy_plugin --strip-components=1 

cp -n %{SOURCE2} KarateSnoopy_plugin/plugin.json

%build

cd KarateSnoopy_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/KarateSnoopy/
cp -r KarateSnoopy_plugin/dist/KarateSnoopy/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/KarateSnoopy/
