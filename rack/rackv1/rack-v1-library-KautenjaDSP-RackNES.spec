...
%global commit0 715f9ff2bfcb0d1a1f5da771692bb2a469c1a619
...
Source0: Rack.tar.gz
Source1: https://github.com/Kautenja/RackNES/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: KautenjaDSP-RackNES_plugin.json
...
%description
KautenjaDSP-RackNES plugin for Rack.
...
mkdir KautenjaDSP-RackNES_plugin
tar xvfz %{SOURCE1} --directory=KautenjaDSP-RackNES_plugin --strip-components=1 

cp -n %{SOURCE2} KautenjaDSP-RackNES_plugin/plugin.json

%build

cd KautenjaDSP-RackNES_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/KautenjaDSP-RackNES/
cp -r KautenjaDSP-RackNES_plugin/dist/KautenjaDSP-RackNES/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/KautenjaDSP-RackNES/
