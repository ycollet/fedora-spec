...
Source0: Rack.tar.gz
Source1: https://github.com/mgunyho/Little-Utils/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: LittleUtils_plugin.json
...
cd LittleUtils_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/LittleUtils/
cp -r LittleUtils_plugin/dist/LittleUtils/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/LittleUtils/
