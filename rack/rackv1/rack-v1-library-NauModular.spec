...
%global commit0 1c15a6c03a4e77c84c2f4dc2c430dec8ff10ad85
...
Source0: Rack.tar.gz
Source1: https://github.com/naus3a/NauModular/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: NauModular_plugin.json
...
mkdir NauModular_plugin
tar xvfz %{SOURCE1} --directory=NauModular_plugin --strip-components=1 

cp -n %{SOURCE2} NauModular_plugin/plugin.json

%build

cd NauModular_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/NauModular/
cp -r NauModular_plugin/dist/NauModular/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/NauModular/
