...
%global commit0 6e530ee693d3f0dff5dfa58212771d63b7e38ab9
...
Source0: Rack.tar.gz
Source1: https://github.com/AnimatedCircuits/RackModules/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: AnimatedCircuits_plugin.json
...
%description
AnimatedCircuits plugin for Rack.
...
mkdir AnimatedCircuits_plugin
tar xvfz %{SOURCE1} --directory=AnimatedCircuits_plugin --strip-components=1 

cp -n %{SOURCE2} AnimatedCircuits_plugin/plugin.json

%build

cd AnimatedCircuits_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AnimatedCircuits/
cp -r AnimatedCircuits_plugin/dist/AnimatedCircuits/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AnimatedCircuits/
