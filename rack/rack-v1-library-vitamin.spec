...
mkdir vitamin_plugin
tar xvfz %{SOURCE1} --directory=vitamin_plugin --strip-components=1 

cp -n %{SOURCE2} vitamin_plugin/plugin.json
...
mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/vitamin/
cp -r vitamin_plugin/dist/vitamin/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/vitamin/
