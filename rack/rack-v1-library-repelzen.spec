...
Source0: Rack.tar.gz
Source1: https://github.com/wiqid/repelzen/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: repelzen_plugin.json
...
%description
repelzen plugin for Rack.
4hp blank panel with logo
...
mkdir repelzen_plugin
tar xvfz %{SOURCE1} --directory=repelzen_plugin --strip-components=1 

cp -n %{SOURCE2} repelzen_plugin/plugin.json
...
mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/repelzen/
cp -r repelzen_plugin/dist/repelzen/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/repelzen/
