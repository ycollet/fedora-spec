...
%global commit0 339c73cde637a61da4a57cd7bedc3878f42ca3f7
...
Source0: Rack.tar.gz
Source1: https://github.com/catronomix/catro-blanco/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: CatroBlanco_plugin.json
...
%description
CatroBlanco plugin for Rack.
2hp decorated blank panel
...
mkdir CatroBlanco_plugin
tar xvfz %{SOURCE1} --directory=CatroBlanco_plugin --strip-components=1 

cp -n %{SOURCE2} CatroBlanco_plugin/plugin.json

%build

cd CatroBlanco_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/CatroBlanco/
cp -r CatroBlanco_plugin/dist/CatroBlanco/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/CatroBlanco/
