...
Name:    rack-v1-AmalgamatedHarmonics
Version: 1.0.4
Release: 3%{?dist}
...
mkdir AmalgamatedHarmonics_plugin
tar xvfz %{SOURCE1} --directory=AmalgamatedHarmonics_plugin --strip-components=1 

cp -n %{SOURCE2} AmalgamatedHarmonics_plugin/plugin.json

%build

cd AmalgamatedHarmonics_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AmalgamatedHarmonics/
cp -r AmalgamatedHarmonics_plugin/dist/AmalgamatedHarmonics/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AmalgamatedHarmonics/
