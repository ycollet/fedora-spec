...
%global commit0 f5a1bf23403b36b395e1ae3d4ee1ea12c22b6546
...
Source0: Rack.tar.gz
Source1: https://github.com/algoritmarte/AlgoritmarteVCVPlugin/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: AlgoritmArte_plugin.json
...
mkdir AlgoritmArte_plugin
tar xvfz %{SOURCE1} --directory=AlgoritmArte_plugin --strip-components=1 

cp -n %{SOURCE2} AlgoritmArte_plugin/plugin.json

%build

cd AlgoritmArte_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AlgoritmArte/
cp -r AlgoritmArte_plugin/dist/AlgoritmArte/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/AlgoritmArte/

%files
%{_libexecdir}/*

%changelog
* Tue Feb 11 2020 Yann Collette <ycollette.nospam@free.fr> - 1.6.1-3
- initial specfile
