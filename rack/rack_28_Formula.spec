...
%global commit0 6638244558048824ac505f385fd5cda70a795cd6
...
License: GPLv2+
URL:     https://github.com/FrankBuss/Formula
...
Source0: Rack.tar.gz
Source1: https://github.com/FrankBuss/Formula/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
...
%description
FrankBussFormula modules for VCV Rack
...
mkdir FrankBussFormula_plugin
tar xvfz %{SOURCE1} --directory=FrankBussFormula_plugin --strip-components=1 

%build

cd FrankBussFormula_plugin
make RACK_DIR=.. DESTDIR=%{buildroot} PREFIX=/usr LIBDIR=%{_lib} %{?_smp_mflags} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack/plugins/FrankBussFormula/
cp -r FrankBussFormula_plugin/dist/FrankBussFormula/* %{buildroot}%{_libexecdir}/Rack/plugins/FrankBussFormula/
