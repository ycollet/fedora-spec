...
%global commit0 574f6b5c8b23edc76edff5856e1352589aebc12b
...
Source0: Rack.tar.gz
Source1: https://github.com/digitalhappens/ILovePerlin/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: ILovePerlin_plugin.json
...
mkdir ILovePerlin_plugin
tar xvfz %{SOURCE1} --directory=ILovePerlin_plugin --strip-components=1 

cp -n %{SOURCE2} ILovePerlin_plugin/plugin.json

%build

cd ILovePerlin_plugin
%make_build RACK_DIR=.. PREFIX=/usr LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ILovePerlin/
cp -r ILovePerlin_plugin/dist/ILovePerlin/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/ILovePerlin/
