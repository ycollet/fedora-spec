...
%global commit0 38075bed27f8496820527200f43bfc7f908efed9
...
Source0: Rack.tar.gz
Source1: https://github.com/max-circlefade/Circlefade/archive/%{commit0}.tar.gz#/%{name}-%{shortcommit0}.tar.gz
Source2: Circlefade_plugin.json
...
%description
Circlefade plugin for Rack.
Random Pitch Gate with Ratcheting
...
mkdir Circlefade_plugin
tar xvfz %{SOURCE1} --directory=Circlefade_plugin --strip-components=1 

cp -n %{SOURCE2} Circlefade_plugin/plugin.json

%build

cd Circlefade_plugin
%make_build RACK_DIR=.. PREFIX=/usr STRIP=true LIBDIR=%{_lib} dist

%install 

mkdir -p %{buildroot}%{_libexecdir}/Rack1/plugins-v1/Circlefade/
cp -r Circlefade_plugin/dist/Circlefade/* %{buildroot}%{_libexecdir}/Rack1/plugins-v1/Circlefade/
